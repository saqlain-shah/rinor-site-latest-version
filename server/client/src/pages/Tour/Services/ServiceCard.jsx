...
const ServiceCard = () => {
  const [allTours, setTour] = useState(tourData);
  // useEffect(() => {
    ...
    <div className="bg-white ">
      <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:gap-x-8">
        {allTours.map((tours) => (
          ...
          >
            <div className="min-h-60 min-w-60 aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-3xl bg-gray-200 lg:aspect-none group-hover:opacity-40 lg:h-80 p-[1px]">
              <img
                src={tours.img}
                alt="Tour"
                className="h-full w-full min-h-60 min-w-60 object-cover object-center rounded-3xl lg:h-full lg:w-full"
              />
            </div>
            ...
            <div className="flex flex-row mr-2 space-x-3 justify-between">
              <p className="text-sm text-left p-2 font-bold">
               Starts from PKR {tours.price["groupof>20perPerson"]}
              </p>
              <button
                type="button"
                data-te-ripple-init
                ...
                onClick={console.log(tours.id)}
              >
                <Link to={`/tours/${tours.id}`}>View Details</Link>
              </button>
            </div>
          </div>
        ))}
